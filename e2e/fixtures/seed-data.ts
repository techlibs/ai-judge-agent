...
function buildProposals(): ProposalSeed[] {
  ...
  for (let i = 1; i <= 5; i++) {
    proposals.push({
      ...
      status: "pending",
      title: `${round1Titles[i - 1] ?? "Proposal"} ${i}`,
      description: `A comprehensive proposal to build ${(round1Titles[i - 1] ?? "project").toLowerCase()} that serves underrepresented communities`,
  ...
  for (let i = 1; i <= 5; i++) {
    proposals.push({
      ...
      fundingRoundId: "round-1",
      status: "evaluated",
      title: `${round1Titles[4 + i] ?? "Proposal"} ${i}`,
      description: `A comprehensive proposal to build ${(round1Titles[4 + i] ?? "project").toLowerCase()} with proven methodology and strong team`,
  ...
  for (let i = 1; i <= 5; i++) {
    proposals.push({
      ...
      fundingRoundId: "round-1",
      status: "funded",
      title: `${round1Titles[9 + i] ?? "Proposal"} ${i}`,
      description: `A comprehensive proposal to build ${(round1Titles[9 + i] ?? "project").toLowerCase()} backed by rigorous technical analysis`,
  ...
  for (let i = 1; i <= 5; i++) {
    proposals.push({
      ...
      status: "disputed",
      title: `${round2Titles[i - 1] ?? "Proposal"} ${i}`,
      description: `A comprehensive proposal to build ${(round2Titles[i - 1] ?? "project").toLowerCase()} currently under community review`,
  ...
  for (let i = 1; i <= 5; i++) {
    proposals.push({
      ...
      fundingRoundId: "round-2",
      status: "evaluated",
      title: `${round2Titles[4 + i] ?? "Proposal"} ${i}`,
      description: `A comprehensive proposal to build ${(round2Titles[4 + i] ?? "project").toLowerCase()} with innovative approach and clear milestones`,
